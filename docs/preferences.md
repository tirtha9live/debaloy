...
- Password `debaloy` (secret) → welcome resident, read-only ledger.
- Password `debaloy_admin4979` (secret) → welcome treasurer, Excel-like editing.
...
- A wrong password locks that client IP for **3 minutes** in KV (`loginlock:`). The Worker rejects further POSTs during the lock. The button shows a countdown and “Try again after some time.” Login submit (click or Enter) otherwise shows “Logging in…”. All of this uses Workers KV + Web Crypto only (no Node APIs).
...
- Client JS is enhancement only (theme, download menu with spinner, dirty-form warning). Saves use form POST.
- Download is one control. Click opens three options: **Excel formatted** (workbook colours and layout from `temp/build_workbook.py`), **Excel raw** (values only), and **Image** (PNG of the current sheet). Excel links still work if JS is off (`/api/export?kind=formatted|raw`).
